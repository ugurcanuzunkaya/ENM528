...
def _solve_internal():
    ...
    for p in P:
        ...
        for (i, j, p_val) in valid_arcs:
            ...
            m.addConstr(load_var[j, p] <= load_var[i, p] - demand_i + BigM * (1 - x[i, j, p]), name=f"LoadProp_{i}_{j}_{p}")
            m.addConstr(load_var[j, p] >= load_var[i, p] - demand_i - BigM * (1 - x[i, j, p]), name=f"LoadProp2_{i}_{j}_{p}")
        ...
        for (i, j, p_val) in valid_arcs:
            ...
            energy_consumption = dist * BaseRate + dist * LoadFactor * (load_var[i, p] - demand_i)
            
            if i in K:
                # Leaving depot: Start with full battery
                m.addConstr(soc_var[j, p] <= BatCap - energy_consumption + BigM * (1 - x[i, j, p]), name=f"SoCProp_Start_{i}_{j}_{p}")
            else:
                # Normal propagation
                m.addConstr(soc_var[j, p] <= soc_var[i, p] - energy_consumption + BigM * (1 - x[i, j, p]), name=f"SoCProp_{i}_{j}_{p}")
