...
def _solve_mobile_vrp_internal():
    ...
    for k in K:
        for p in range(NumP):
            ...
            for i in N:
                for j in N:
                    if i != j:
                        x[i, j, k, p] = m.addVar(vtype=GRB.BINARY, name=f"x_{i}_{j}_{k}_{p}")
            # End arcs
            for i in N:
                x[i, Sink, k, p] = m.addVar(vtype=GRB.BINARY, name=f"x_{i}_Sink_{k}_{p}")
    ...
    # 2. Truck Route Cost
    cost_truck = gp.quicksum(d_ij[u, v] * z[u, v] for u in T_nodes for v in T_nodes if u != v)
    ...
    for k in K:
        m.addConstr(gp.quicksum(z[u, k] for u in T_nodes if u != k) == y[k], name=f"Truck_In_{k}")
        m.addConstr(gp.quicksum(z[k, v] for v in T_nodes if v != k) == y[k], name=f"Truck_Out_{k}")
    ...
    for i in K:
        for j in K:
            if i != j:
                m.addConstr(u_truck[i] - u_truck[j] + len(K) * z[i, j] <= len(K) - 1, name=f"Truck_MTZ_{i}_{j}")
    ...
    for i in N:
        m.addConstr(
            gp.quicksum(x[u, i, k, p] 
                        for k in K for p in range(NumP) 
                        for u in ([k] + N) if (u, i, k, p) in x) == 1,
    ...
    for k in K:
        for p in range(NumP):
            ...
            for j in N:
                inflow = gp.quicksum(x[u, j, k, p] for u in ([k] + N) if (u, j, k, p) in x)
                outflow = gp.quicksum(x[j, v, k, p] for v in (N + [Sink]) if (j, v, k, p) in x)
    ...
    for i in N:
        for j in N:
            if i != j:
                ...
                m.addConstr(u_pers[i] - u_pers[j] + BigM * sum_x <= BigM - 1, name=f"Pers_MTZ_{i}_{j}")
...
def plot_solution(data, model, x, z, y, save_path=None):
    ...
    # Plot Main Depot
    plt.scatter(coords[O][0], coords[O][1], c='black', marker='^', s=250, label='Main Depot', zorder=5)
    ...
    for k in K:
        ...
        plt.scatter(coords[k][0], coords[k][1], c=color, marker='s', s=size, label=label, zorder=4)
        ...
        for v in T_nodes:
            if u != v and z[u, v].X > 0.5:
                plt.plot([coords[u][0], coords[v][0]], [coords[u][1], coords[v][1]], 
                         c='black', lw=3, linestyle='--', zorder=2, label='Truck Route' if u==O else "")
