...
def generate_data() -> Dict[str, Any]:
    """
    Generates data for the Mobile VRP problem.
    """
    ...
    # Candidates: 100, 101, 102 (Using distinct IDs to avoid confusion)
    ...
    # Main Depot Coordinate (For visualization purposes only - distances are parameters)
    main_depot_coords = (50, 150)

    # Fixed locations for candidates to make optimization meaningful
    coords[100] = (20, 80)
    coords[101] = (50, 80)
    coords[102] = (80, 20)
...
def solve_mobile_vrp():
    ...
    with open("output_project.txt", "w") as f:
        sys.stdout = f
        try:
            _solve_mobile_vrp_internal()
        finally:
            sys.stdout = original_stdout
    print("Output saved to output_project.txt")
...
def _solve_mobile_vrp_internal():
    ...
    # We only need arcs:
    # - Between customers (i, j in N)
    # - From candidate to customer (k in K, j in N)
    # - From customer to candidate (i in N, k in K)
    valid_arcs = []
    for p in P:
        for i in N:
            for j in N:
        ...
        for k in K:
            for j in N:
                valid_arcs.append((k, j, p))  # Depot to Customer
                valid_arcs.append((j, k, p))  # Customer to Depot
    ...
    # Z_max: Variable for Minimax (Minimize the maximum route length)
    ...
    # We want to minimize the maximum route length among all personnel
    ...
    # Each customer visited exactly once by one personnel
    for i in N:
        m.addConstr(
    ...
    for p in P:
        ...
        for k in K:
            ...
            # Flow balance at depot: If leave k, must return to k
            inflow_k = gp.quicksum(x[j, k, p] for j in N)
            m.addConstr(outflow_k == inflow_k, name=f"Depot_Balance_{k}_{p}")

        # Vehicle starts from at most one depot
        # FORCE usage of all personnel: Must start exactly once
        m.addConstr(
            gp.quicksum(x[k, j, p] for k in K for j in N) == 1, name=f"Force_Start_{p}"
        )

    # 5. Max Route Length & Minimax Constraint
    for p in P:
        # Filter arcs for THIS personnel p to avoid summing all personnel
    ...
    if m.status == GRB.OPTIMAL:
        ...
        for p in P:
            ...
            # Check if this personnel is used
            used = False
            for j in N:
...
def plot_solution(data, model, x, y, save_path=None):
    ...
    if selected_k is not None:
        plt.plot(
            [main_depot_coords[0], coords[selected_k][0]],
    ...
    plt.title("Optimal Mobile VRP Routes with Truck Logistics")
    plt.legend()
    plt.grid(True)
    if save_path:
