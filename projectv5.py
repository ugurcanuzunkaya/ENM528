...
def solve_weighted_sum(alpha, data):
    ...
    route_lens = {}
    for p in P:
        route_lens[p] = gp.quicksum(
            d_ij[i, j] * x[i, j, p] for (i, j, p_val) in valid_arcs if p_val == p
        )
    ...
    for p in P:
        m.addConstr(
            gp.quicksum(x[k, j, p] for k in K for j in N) == 1, name=f"Force_Use_{p}"
        )
...
def solve_mobile_vrp():
    ...
    with open("output_projectv5.txt", "w") as f:
        ...
        print(
            f"{'Alpha':<10} | {'Z1 (Cost)':<15} | {'Z2 (Balance)':<15} | {'ObjVal':<15}"
        )
        print("-" * 65)
